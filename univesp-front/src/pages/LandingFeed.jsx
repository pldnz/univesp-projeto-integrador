...
import { Heart, MapPin, Filter, Search, ChevronDown, Sparkles } from 'lucide-react';
      ...
      <main className="container pt-20">
        ...
        <div className="flex flex-col md:flex-row gap-8 mb-20 items-center">
          <div className="flex-1 w-full relative group">
            <Search className="absolute left-6 top-1/2 -translate-y-1/2 text-tertiary/40 group-focus-within:text-primary transition-colors" size={20} />
            <input 
              type="text" 
          ...
          >
            <Filter size={20} /> {showFilters ? 'Recolher Filtros' : 'Filtros Avançados'}
          </button>
        </div>
        ...
        <div className="mb-16 flex flex-col md:flex-row justify-between items-end gap-10">
          <div>
            <h2 className="text-5xl md:text-7xl font-black text-on-surface tracking-tighter leading-none mb-6">
              Nossos <br /><span className="text-primary italic">Amigos.</span>
            </h2>
        ...
        {showFilters && (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-20 animate-fade-in">
            <div className="space-y-3">
              <label className="text-[10px] font-black uppercase tracking-widest text-primary/40 ml-1">Sexo</label>
              <select 
                value={gender} 
            ...
            <div className="space-y-3">
              <label className="text-[10px] font-black uppercase tracking-widest text-primary/40 ml-1">Porte</label>
              <select 
                value={size} 
            ...
            <div className="space-y-3">
              <label className="text-[10px] font-black uppercase tracking-widest text-primary/40 ml-1">Idade</label>
              <select 
                value={ageGroup} 
        ...
        {loading ? (
            ...
            {pets.map((pet) => (
              ...
              >
                ...
                <div className="px-2">
                  ...
                  <p className="text-[10px] font-black text-primary uppercase tracking-[0.2em] mb-4 line-clamp-1">{pet.institution_name}</p>
                  <div className="flex items-center gap-2 text-tertiary mb-6">
                    <MapPin size={14} className="text-primary" />
                    <span className="text-xs font-bold uppercase tracking-wider">{pet.location}</span>
                  </div>
                  <div className="flex gap-2">
                    <span className="chip uppercase">{pet.gender}</span>
                    <span className="chip bg-surface-container-highest text-tertiary uppercase">{pet.size}</span>
                  </div>
                </div>
              </Link>
            ))}
          ...
          <div className="py-32 text-center section-tonal border-2 border-dashed border-primary/10">
            ...
            <h2 className="text-4xl font-black mb-4 italic">Nenhum amigo encontrado.</h2>
            <p className="text-tertiary font-medium mb-12">Tente ajustar seus filtros ou pesquisar por outra cidade.</p>
            <button onClick={clearFilters} className="btn-primary">Ver Todos os Animais</button>
          </div>
        )}
      </main>
    </div>
